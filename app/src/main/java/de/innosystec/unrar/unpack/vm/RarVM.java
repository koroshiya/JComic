...
public class RarVM extends BitInput {
	...
	public void execute(VMPreparedProgram prg) {
		...
		long staticSize = Math.min(prg.getStaticData().size(), VM_GLOBALMEMSIZE - globalSize);
		if (staticSize != 0) {
		...
		while (true) {
			...
			case VM_CMP: {
				...
				if (result == 0) {
					...
					flags = (result > value1) ? 1 : result & VMFlags.VM_FS.getFlag();
				}
			}
				break;
			...
			case VM_ADD: {
				int value1 = getValue(cmd.isByteMode(), mem, op1);
				int result = (int) ((long) value1 + (long) getValue(cmd.isByteMode(), mem, op2));
				if (cmd.isByteMode()) {
					result &= 0xff;
			...
			case VM_SUB: {
				...
				flags = (result == 0) ? VMFlags.VM_FZ.getFlag()
						: (result > value1) ? 1 : (result & VMFlags.VM_FS.getFlag());
			...
			case VM_XCHG: {
				...
				break;
			case VM_MUL: {
				int result = (int) (((long) getValue(cmd.isByteMode(), mem, op1) & 0xFFffFFff * (long) getValue(cmd.isByteMode(), mem, op2)));
				setValue(cmd.isByteMode(), mem, op1, result);
			}
				break;
			case VM_DIV: {
			...
			case VM_ADC: {
				...
				int result = (int) ((long) value1 & 0xFFffFFff + (long) getValue(cmd.isByteMode(), mem, op2) & 0xFFffFFff + (long) FC);
				if (cmd.isByteMode()) {
					result &= 0xff;
				}
			...
			case VM_SBB: {
				...
				flags = (result > value1 || result == value1 && FC != 0) ? 1 : (result == 0 ? VMFlags.VM_FZ.getFlag() : (result & VMFlags.VM_FS.getFlag()));
				setValue(cmd.isByteMode(), mem, op1, result);
			}
				break;
		...
		for (VMPreparedCommand cmd : commands) {
			...
			for (int i = commands.indexOf(cmd) + 1; i < commands.size(); i++) {
				...
				if ((flags & (VMCmdFlags.VMCF_JUMP | VMCmdFlags.VMCF_PROC | VMCmdFlags.VMCF_USEFLAGS)) != 0) {
					flagsRequired = true;
					break;
				}else if ((flags & VMCmdFlags.VMCF_CHFLAGS) != 0) {
					break;
				}
			}
			if (flagsRequired) {
	...
	private VMStandardFilters IsStandardFilter(byte[] code) {
		...
		int CodeCRC = RarCRC.checkCrc(0xffffffff,code,0,code.length);
        for (VMStandardFilterSignature i : stdList){
			if (i.getCRC()==CodeCRC && i.getLength()==code.length){
				return(i.getType());
			}
		}
		              ...
		              for (int i=0;i<=2;i++) {
                          if ((cmdMask & (1 << i)) != 0) {
                              ...
                              if (filterItanium_GetBits(curPos, startPos + 37, 4) == 5) {
                                  filterItanium_SetBits(curPos, (int) (filterItanium_GetBits(curPos, startPos + 13, 20) - fileOffset) & 0xfffff, startPos + 13);
                              }
                          }
                      }
		          }
		    ...
		    case VMSF_RGB:
		        ...
		        for (int curChannel=0;curChannel<channels;curChannel++){
		          ...
		          for (int i=curChannel;i<dataSize;i+=channels){
		            ...
		            if (upperPos>=3){
		              int upperDataPos=dataSize+upperPos;
		            ...
		            mem[dataSize+i]=(byte)(prevByte&0xff);
		           
		          }
		        }
                border=dataSize-2;
		        for (int i = posR; i < border; i += 3){
		          byte G=mem[dataSize+i+1];
		          mem[dataSize+i]+=G;
		          mem[dataSize+i+2]+=G;
		        }
		      break;
