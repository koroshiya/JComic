...
public class ImageParser {
    ...
    public static final FilenameFilter fnf = (dir, filename) -> {
        File f = new File(dir, filename);
        return f.isFile() && f.canRead() && f.length() > 0 && isSupportedImage(f);
    };
    public static final FilenameFilter fullFnf = (dir, filename) -> {
        File f = new File(dir, filename);
    ...
    private static Point getPNGSize(File f) throws IOException {
        ...
        int bRead = is.read(b137, 0, 24);
        is.close();
        Log.d("ImageParser", String.format(Locale.getDefault(), "Read %d bytes", bRead));
