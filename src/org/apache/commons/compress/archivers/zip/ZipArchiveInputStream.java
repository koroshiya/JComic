...
public class ZipArchiveInputStream extends ArchiveInputStream {
    ...
    */
    ...
    public int read(byte[] buffer, int offset, int length) throws IOException {
        ...
        int read;
        if (current.entry.getMethod() == STORED) {
            read = readStored(buffer, offset, length);
        } else if (current.entry.getMethod() == DEFLATED) {
    ...
    private void closeEntry() throws IOException {
        ...
        if (current.bytesReadFromStream <= current.entry.getCompressedSize()
            ...
            long inB = current.entry.getMethod() == DEFLATED ? getBytesInflated() : current.bytesRead;
