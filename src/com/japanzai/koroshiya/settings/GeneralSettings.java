...
public class GeneralSettings extends SettingTab {
	...
	private void instantiate(){
		...
		resetAll.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT,
	...
	public void save(){
		...
		settings.setLoopMode(loop.getState() == 1);
		settings.setSaveSession(previousSession.getState() == 1);
		settings.setSaveRecent(recent.getState() == 1);
		settings.setBacklightAlwaysOn(backlightOn.getState() == 1);
		settings.setKeepZoomOnPageChange(keepZoom.getState() == 1);
		settings.setContextMenuEnabled(contextMenu.getState() == 1);
		settings.setBackToFileChooserEnabled(backToFileChooser.getState() == 1);
		settings.setZoomIndex(defaultZoom.getState());
		settings.setOrientationIndex(orientation.getState());
		settings.setDoubleTapIndex(doubleTap.getState());
		
	}
	
	public void load(){

		loop.setState(settings.isLoopModeEnabled() ? 1 : 0);
		previousSession.setState(settings.saveSession() ? 1 : 0);
		recent.setState(settings.saveRecent() ? 1 : 0);
		backlightOn.setState(settings.isBacklightAlwaysOn() ? 1 : 0);
		keepZoom.setState(settings.keepZoomOnPageChange() ? 1 : 0);
		contextMenu.setState(settings.isContextMenuEnabled() ? 1 : 0);
		backToFileChooser.setState(settings.isBackToFileChooser() ? 1 : 0);
		defaultZoom.setState(settings.getZoom());
		orientation.setState(settings.getOrientation());
		doubleTap.setState(settings.getDoubleTapIndex());
