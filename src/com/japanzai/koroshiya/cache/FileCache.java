...
public class FileCache extends Steppable {
	...
	public FileCache(MainActivity parent, String path){
		
		super(parent, path);
		
		setPrimary(new PrimaryThread(this, true));
		setSecondary(new SecondaryThread(this, false));
	...
	public JBitmapDrawable parseImage(int findex) {
		...
		try{
			...
			JBitmapDrawable temp = ImageParser.parseImageFromDisk(is, size.x, size.y, name);
			if (temp == null){
				super.clear();
				is = new FileInputStream((String)getImages().get(findex).getImage());
				temp = ImageParser.parseImageFromDisk(is, size.x, size.y, name);
			}
			return temp;
	...
	public StepThread getNextThread(boolean primary) {
		return primary ? new PrimaryThread(this, true) : new SecondaryThread(this, true);
	}

	@Override
	public StepThread getPreviousThread(boolean primary) {
		return primary ? new PrimaryThread(this, false) : new SecondaryThread(this, false);
	}
